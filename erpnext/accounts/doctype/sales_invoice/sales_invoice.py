...
class SalesInvoice(SellingController):
	...
	def before_save(self):
		print("============On submit Sales Invoice=============")
		customer = frappe.get_doc("Customer", self.customer)
		company = frappe.get_doc("Company", self.company)
		fiscal_year = frappe.db.get_value("Fiscal Year Company", filters={"company": self.company}, fieldname="*")

		print(customer.tax_id)
		data = {
				"username":"Test_CBMS", 
				"password":"test@321", 
				"seller_pan":company.tax_id,
				"buyer_pan":customer.tax_id,
				"buyer_name":customer.customer_name, 
				"fiscal_year":fiscal_year.parent, 
				"invoice_number":self.name,
				"invoice_date":self.posting_date, 
				"total_sales":self.grand_total, 
				"taxable_sales_vat":self.total, 
				"vat":self.total_taxes_and_charges, 
				"excisable_amount":0, 
				"excise":0,
				"taxable_sales_hst":0,
				"hst":0, 
				...
				"isrealtime":"true", 
				"datetimeclient":datetime.datetime.now().strftime("%d-%b-%Y %H:%M:%S")
		}
		# api-endpoint 
	...
	def on_submit(self):
		...
		if r.status_code == 200:
			...
			frappe.db.sql("""update `tabSales Invoice`
				set status="Submitted and IRD Posted"
